...
// --------------- serde_adaptor<rapidjson::Value, rttr::variant, detail::basic_t> ---------------
void serde_adaptor<rapidjson::Value, rttr::variant, detail::basic_t>::from(rapidjson_value& s, std::string_view key, ValueType& data) {
    ValueType val;
...
void serde_adaptor<rapidjson::Value, rttr::variant, detail::basic_t>::into(rapidjson_value& s, std::string_view key, const ValueType& orig_data) {
...
// --------------- serde_adaptor<rapidjson::Value, rttr::variant, detail::basic_t> ---------------
void serde_adaptor<rapidjson::Document, rttr::variant, detail::basic_t>::from(rapidjson_type& s, std::string_view key, ValueType& data) {
    auto& avalue = key.empty() ? s : s[key.data()];
    ValueType val;
    if (avalue.IsBool()) {
        val = avalue.GetBool();
...
void serde_adaptor<rapidjson::Document, rttr::variant, detail::basic_t>::into(rapidjson_type& s, std::string_view key, const ValueType& orig_data) {
    const rttr::type orig_data_type = orig_data.get_type();
    rttr::variant wrapped_val = orig_data_type.is_wrapper() 
            ? orig_data.extract_wrapped_ptr_value() : orig_data;
    const rttr::type wrapped_type = wrapped_val.get_type();
...
// --------------- serde_adaptor<rapidjson::Value, rttr::variant, detail::basic_t> ---------------
void serde_adaptor<rapidjson::Document, rttr::variant_polymoph_view, detail::poly_t>::from(rapidjson_type& s, std::string_view key, Container& container) {
...
void serde_adaptor<rapidjson::Document, rttr::variant_polymoph_view, detail::poly_t>::into(rapidjson_type& s, std::string_view key, const Container& data) {
...
// --------------- serde_adaptor<rapidjson::Document, rttr::variant_sequential_view, detail::seq_t> ---------------
void serde_adaptor<rapidjson::Document, rttr::variant_sequential_view, detail::seq_t>::from(rapidjson_type& s, std::string_view key, Sequent& seq) {
    auto& table = key.empty() ? s : s[key.data()];
...
void serde_adaptor<rapidjson::Document, rttr::variant_sequential_view, detail::seq_t>::into(rapidjson_type& s, std::string_view key, const Sequent& data) {
    using namespace rapidjson;
    auto& alloc = s.GetAllocator();
    rapidjson::Value arr;
    arr.SetArray();
...
// --------------- serde_adaptor<rapidjson::Document, rttr::variant_associative_view, detail::map_t> ---------------
void serde_adaptor<rapidjson::Document, rttr::variant_associative_view, detail::map_t>::from(rapidjson_type& s, std::string_view key, Map& map) {
    auto& table = key.empty() ? s : s[key.data()];
...
void serde_adaptor<rapidjson::Document, rttr::variant_associative_view, detail::map_t>::into(rapidjson_type& s, std::string_view key, const Map& data) {
    using namespace rapidjson;
    Value map;
    auto& alloc = s.GetAllocator();
...
// --------------- serde_adaptor<rapidjson::Document, rttr::variant, detail::struct_t> ---------------
void serde_adaptor<rapidjson::Document, rttr::variant, detail::struct_t>::from(rapidjson_type& s, std::string_view key, Object& orig_data) {
...
void serde_adaptor<rapidjson::Document, rttr::variant, detail::struct_t>::into(rapidjson_type& s, std::string_view key, const Object& _data) {
    using namespace rapidjson;
...
// --------------- serde_adaptor<rapidjson::Value, rttr::variant_polymoph_view, detail::poly_t> ---------------
void serde_adaptor<rapidjson::Value, rttr::variant_polymoph_view, detail::poly_t>::from(rapidjson_value& s, std::string_view key, Container& container) {
    auto& table = key.empty() ? s : s[key.data()];
    if (!table.IsObject()) { return; }
...
// --------------- serde_adaptor<rapidjson::Value, rttr::variant_sequential_view, detail::seq_t> ---------------
void serde_adaptor<rapidjson::Value, rttr::variant_sequential_view, detail::seq_t>::from(rapidjson_value& s, std::string_view key, Sequent& seq) {
    auto& table = key.empty() ? s : s[key.data()];
...
// --------------- serde_adaptor<rapidjson::Value, rttr::variant_associative_view, detail::map_t> ---------------
void serde_adaptor<rapidjson::Value, rttr::variant_associative_view, detail::map_t>::from(rapidjson_value& s, std::string_view key, Map& map) {
    auto& table = key.empty() ? s : s[key.data()];
    if (map.is_key_only_type()) {
        const rttr::type orig_key_type = map.get_key_type();
        if (detail::rttr_type_is_basic(orig_key_type)) {
...
// --------------- serde_adaptor<rapidjson::Value, rttr::variant, detail::struct_t> ---------------
void serde_adaptor<rapidjson::Value, rttr::variant, detail::struct_t>::from(rapidjson_value& s, std::string_view key, Object& orig_data) {
    rttr::type orig_type = orig_data.get_type();
    rttr::instance data(orig_data);
    rttr::instance obj = data.get_type().get_raw_type().is_wrapper() 
            ? data.get_wrapped_instance() : data;
