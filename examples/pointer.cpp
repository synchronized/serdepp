...
int main(int argc, char *argv[])
{
    ...
    static_assert(serde::detail::is_pointer_v<std::unique_ptr<int>>);
    static_assert(serde::detail::is_pointer_v<std::shared_ptr<int>>);

    //std::unique_ptr<int> i = serde::is_pointer<std::unique_ptr<int>>::null_type;
    std::unique_ptr<int> i = serde::detail::is_pointer<std::unique_ptr<int>>::init();
    *i = 2;
        
    int* j = new int();
    *j = 1;

    std::shared_ptr<int> k = serde::detail::is_pointer<std::shared_ptr<int>>::init();
    *k = 3;
